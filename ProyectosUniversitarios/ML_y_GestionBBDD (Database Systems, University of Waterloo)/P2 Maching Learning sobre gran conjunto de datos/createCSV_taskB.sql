...
class AS (
    SELECT playerID,
    CASE
        WHEN playerID IN (SELECT * FROM players_inducted) THEN 1
        WHEN playerID IN (SELECT * FROM players_not_inducted) THEN 0
    END AS class
    FROM (SELECT * FROM players_inducted UNION SELECT * FROM players_not_inducted) AS HoF_players
),
...
INTO OUTFILE '/var/lib/mysql-files/taskB.csv'
